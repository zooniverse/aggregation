...
img1 = cv2.imread('/home/ggdhines/Databases/old_weather/images/Bear-AG-29-1939-0185.JPG')          # queryImage
img2 = cv2.imread('/home/ggdhines/Databases/old_weather/images/Bear-AG-29-1939-0175.JPG') # trainImage
...
P = np.mat(np.array(image1_pts)).transpose()
Q = np.mat(np.array(image2_pts)).transpose()
s = P*(P.transpose())
s_inv = inv(s)
...
for p,q in zip(P.transpose(),Q.transpose()):
    ...
    print math.sqrt((p_x-q_x)**2+(p_y-q_y)**2)
    # print H
    # print
    t = H*p.transpose()

    t_x,t_y = t[(0,0)],t[(1,0)]
    print math.sqrt((t_x-q_x)**2+(t_y-q_y)**2)
