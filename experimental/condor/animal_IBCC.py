...
for subject_index,subject in enumerate(random.sample(to_sample_from,3000)):
    print subject_index
...
prior = np.mean([np.mean(v) for v in animal_votes])
f.close()
...
with open(base_directory+"/Databases/condor_ibcc.py","wb") as f:
    ...
    f.write("nu0 = np.array(["+str(int((1-prior)*100))+","+str(int(prior*100))+"])\n")
