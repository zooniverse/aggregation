...
class Photo:
    ...
    def __canAdd__(self):
        return len(self.users) < 15
        #return True

    def __updateLikelihood__(self):
        oldContains = self.contains[:]
        for species in speciesList:
            self.likelihood[species] = reduce(operator.mul, [user.__getProbability__(self,species) for user in self.users], 1)
            #self.likelihood[species] = np.mean([user.__getProbability__(self,species) for user in self.users])

        sortedLikelihoods = sorted(self.likelihood.items(), key=lambda x:x[1],reverse=True)
        self.contains = []
        for i in range(len(sortedLikelihoods)):
            if sortedLikelihoods[i][1] < 0.3:#0.6*sortedLikelihoods[0][1]:
                break
            self.contains.append(sortedLikelihoods[i][0])

        # if  (sorted(self.contains) != sorted(oldContains)):
        #     print sortedLikelihoods[1][1]
        #     print 0.5*sortedLikelihoods[0][1]
        #     print sorted(self.contains)
        #     print sorted(oldContains)
        #     print self.goldStandard
        #     #print self.__majorityVote__()
        #     print sortedLikelihoods[:5]
        #     #print [u.__getProbability__(self,"wildebeest") for u in self.users]
        #     #print [u.__getClassification__(self) for u in self.users]
        #     print "==--"
        #     assert False
        return (sorted(self.contains) != sorted(oldContains))
    ...
    def __weightedMajorityVote__(self):
        ...
        for species in speciesList:
            ...
            weights = {u:u.speciesCorrect[species]**21 for u in enoughVotes}
...
# Y = []
# for u in users.values():
#     e = u.__speciesCorrect__("gazelleThomsons")
#     if (e == -1):
#         continue
#
#     e2 = u.__speciesCorrect__("gazelleGrants")
#     if (e2 == -1):
#         continue
#
#
