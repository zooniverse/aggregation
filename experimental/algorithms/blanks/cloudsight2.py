...
counts_as_animal = ["horse","hen","peafowl","turkeys","peacock","peafowls","gazel","lion","board","antilope","elk","peacocks","cows","hyene","hyena","truck","buffalo","van","dear","pig","elephant","deer","animals","monkey","animal","gazelle","antelope","bird","land rover","suv","giraffe","goat","coyote","s.u.v","doe","man","person","lionesses","zebra","hyenas","jeep","car","sub","oryx","mammal"]
...
for subject_id in subjects:
    subject = subject_collection.find_one({"zooniverse_id":subject_id})
    if subject["metadata"]["retire_reason"] == "blank_consensus":
        continue

    urls = []


    for i in subject["location"]["standard"]:
    ...
    if False not in [u in token_mapping for u in urls]:
        ...
        if "blank" in subject["metadata"]["counters"]:
            print subject["metadata"]["counters"]["blank"]/float(sum(subject["metadata"]["counters"].values()))
        else:
            print 0
        ...
        for u in urls:
            ...
            try:
                for a in counts_as_animal:
                    found_animal = found_animal or (a in t["name"])
