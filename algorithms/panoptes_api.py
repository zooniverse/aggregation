...
class PanoptesAPI:
    ...
    def __get_subject_ids__(self):
        ...
        cur = self.postgres_session.cursor()
        cur.execute(select)
        subject_set_id = cur.fetchone()[0]

        select = "SELECT subject_id FROM set_member_subjects WHERE subject_set_id = " + str(subject_set_id)
        cur = self.postgres_session.cursor()
        cur.execute(select)
        subjects = cur.fetchall()
        self.subjects = [s[0] for s in subjects]
                ...
                for tool in tasks[task_id]["tools"]:
                    ...
                    if "details" in self.classification_tasks[task_id][shape]:
                        ...
                        if tool["details"] != []:
                            ...
                            self.classification_tasks[task_id][shape]["details"] = [i for i in range(len(tool["details"]))]
                ...
                if self.task_type[task_id] == "drawing":
                    for drawing in task["value"]:
                        ...
                        if id_ not in drawings:
                            ...
                            drawings[id_].append((user_id,relevant_params))
    ...
    def __sort_classifications__(self):
        ...
        for subject_id in self.subjects:
            ...
            for record in self.cassandra_session.execute("select user_id,annotations from classifications where project_id = " + str(self.project_id) + " and subject_id = " + str(subject_id)):
                # convert from string into json
                        ...
                        if task_id in self.marking_params_per_task:
                            ...
                            for marking in task["value"]:
                                ...
                                for question_id,classification in enumerate(marking["details"]):
                                    t_classifications[task_id][cluster_index][question_id].append((record.user_id,classification["value"]))
                        # else we have a simple classification - makes things so much easier :)
                        else:
                            if task_id not in t_classifications:
                                t_classifications[task_id] = []

                            t_classifications[task_id].append((record.user_id,task["value"]))

            self.classifications[subject_id] = deepcopy(t_classifications)
    def __classify__(self):
        self.__sort_classifications__()
        assert False
    ...
    def __get_classifications__(self,subject_id,task_id,(cluster_index,question_id)=(None,None)):
...
if __name__ == "__main__":
    project = PanoptesAPI("kitteh-zoo")
    # project.__migrate__()
